(function() {
	...
	app
			.controller(
					'insuranceStepperController',
					function($scope, insuranceService, ngNotify, $state) {
						...
						function init() {
							...
							insuranceService
									.getTravelInsuranceRisks("Roadside")
									.then(
											function(response) {
												if (response.status == 200) {
													console.log(response.data);
						...
						vm.calculateDays = function calculateDays(fromDate,
								toDate) {
							var timeDiff = Math.abs(fromDate.getTime()
									- toDate.getTime());
						...
						vm.submitCurrentStep = function submitCurrentStep() {
							// validation goes here , if its satisfied then it's
							// this
							switch (vm.selectedStep) {
							case 0: {
								vm.stepOne.completed = true;
								vm.addTabs();
							}
								break;
							case 1:
								vm.stepTwo.completed = true;
								break;
							case 2:
								vm.stepThree.completed = true;
								break;
							case 3:
								vm.stepFour.completed = true;
								break;
							}
						...
						vm.createInsurancePolicy = function() {
							...
							var internationalTravelInsuranceDTO = {
								...
								"durationInDays" : vm.calculateDays(
										vm.stepOne.data.fromDate,
										vm.stepOne.data.toDate),
							...
							if (!vm.stepFour.isSkiped) {
								var roadsideRisks = [];
								roadsideRisks
										.push(vm.stepFour.data.selectedAccommodation);
								roadsideRisks
										.push(vm.stepFour.data.selectedRepair);
								roadsideRisks
										.push(vm.stepFour.data.selectedTowing);
								roadsideRisks
										.push(vm.stepFour.data.selectedTransport);
							...
							insuranceService
									.createInsurancePolicy(insurancePolicyDTO)
									.then(
											function(response) {
												if (response.status == 200) {
													ngNotify
